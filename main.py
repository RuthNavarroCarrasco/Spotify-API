...
def get_top_artists():
    ...
    # Si prefieres devolver el JSON directamente:
    # return jsonify(top_artists)

    # Si prefieres mostrar la información en formato tabla:
    table = '<table border="1">'
    table += '<tr><th>Artist Name</th><th>Genres</th><th>Followers</th><th>Popularity</th><th>Image</th><th>Spotify Link</th></tr>'

    for artist in top_artists['items']:
        genres = ', '.join(artist['genres']) if artist['genres'] else 'N/A'
        table += '<tr>'
        table += f"<td>{artist['name']}</td>"
        table += f"<td>{genres}</td>"
        table += f"<td>{artist['followers']['total']}</td>"
        table += f"<td>{artist['popularity']}</td>"
        table += f"<td><img src='{artist['images'][0]['url']}' alt='{artist['name']}' width='100'></td>"
        table += f"<td><a href='{artist['external_urls']['spotify']}'>Open in Spotify</a></td>"
        table += '</tr>'
